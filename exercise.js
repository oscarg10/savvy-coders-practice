...
//Pass "getTLD" Function to get the letters after "."

const TLDs = users
  // You can only destructor objects, not strings
...
// const TLDsEMails = users.map(
//   ({ website, email }) => `${getTLD(website)} ${getTLD(email)}`
// );
// console.log(TLDsEMails);
const TLDs = users
  .map(({ website }) => getTLD(website))
  .reduce((tally, currentTLD) => {
    // Ternary operators -> condition ? if so, do this : if not, do this.
    tally[currentTLD] ? (tally[currentTLD] += 1) : (tally[currentTLD] = 1);
    return tally;
  }, {});
